...
- JSX data display 提取已过滤状态/事件/配置类 prop binding，避免 `open={confirmOpen}`、`options={users}` 这类状态变量或数据源进入 unmatched displays。
- 已新增 9 条组件字段单元参数化测试、1 条 Modal 单元测试、10 条 e2e 参数化样例。
