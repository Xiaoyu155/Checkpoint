...
def workflow_atomic_capabilities() -> tuple[Capability, ...]:
    specs = [
        ...
        Capability(
            ...
            kind="action",
            available=True,
            description="Capture a desktop screenshot, use OmniParser-compatible visual detection, and click the matched element.",
            input_schema={
                "type": "object",
        ...
        Capability(
            name="assert_visual_text",
            kind="assertion",
            available=True,
            description="Capture a desktop screenshot with OmniParser-compatible detection and assert that the visible text exists in the matched region.",
            input_schema={
                "type": "object",
                "required": ["text"],
                "fields": {
        ...
        Capability(
            ...
            kind="command",
            available=True,
            description="Capture a value from text or inputs into a named workflow variable.",
            input_schema={"type": "object", "required": ["name"], "fields": {"name": "string", "value": "any?", "value_from": "input.path?", "from_text": "string?"}},
            output_schema={"type": "object", "fields": {"name": "string", "value": "any"}},
            dry_run_supported=True,
            risk_level="low",
            planner_visible=True,
        ),
        Capability(
            ...
            kind="command",
            available=True,
            description="Branch workflow execution based on whether the latest observation contains text.",
            input_schema={"type": "object", "required": ["text"], "fields": {"text": "string", "then": "string?", "else": "string?", "observation": "string?"}},
            output_schema={"type": "object", "fields": {"branch_taken": "then|else", "jump_to": "string?"}},
            dry_run_supported=True,
            risk_level="low",
            planner_visible=True,
        ),
        Capability(
            ...
            kind="command",
            available=True,
            description="Run another workflow as a nested sub-workflow.",
            input_schema={"type": "object", "required": ["workflow"], "fields": {"workflow": "string", "inputs": "object?", "dry_run": "boolean?", "from_step": "string?"}},
            output_schema={"type": "object", "fields": {"nested_run": "WorkflowRunResult"}},
            dry_run_supported=True,
            risk_level="low",
            planner_visible=True,
        ),
...
def command_capabilities() -> tuple[Capability, ...]:
    commands = {
        ...
        "generate-report": "Generate a static HTML run history report.",
