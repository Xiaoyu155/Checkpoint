...
from .cli_workspace_record import WORKSPACE_RECORD_COMMANDS
from .cli_workspace_run import WORKSPACE_RUN_COMMANDS
...
def build_parser(prog: str = DEFAULT_CLI_NAME) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog=prog,
        description=(
            "Checkpoint local verification CLI. The package and legacy command are named visual-agent; "
            "checkpoint is the preferred brand-aligned alias."
        ),
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command in WORKSPACE_READ_COMMANDS:
        from .cli_workspace_read import handle_workspace_read_command
