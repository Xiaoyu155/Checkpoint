...
> 当前分支：`main`  
> 当前工作区状态：Phase 3 cloud feature gate 已实现并验证
> 最新代码提交：本轮收尾提交见 `git log -1`
...
- JSX data display 提取已排除 `open={...}`、`checked={...}`、事件 handler、`options/dataSource/items/data/columns`、React Hook Form `register/control/render/rules` 等状态/配置 prop，避免把 Modal/Switch/Autocomplete 状态和表单 helper 误判为页面展示数据；保留 `value={profile.displayName}` 这类 unmatched display 诊断。
...
- 服务端支持 `GET /v1/health`、`POST /v1/run`、`GET /v1/runs`、`GET /v1/run/{run_id}`。
