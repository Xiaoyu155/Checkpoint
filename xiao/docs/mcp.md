...
Those variables configure the client. The hosted Cloud API uses separate server-side settings: configure both `VISUAL_AGENT_CLOUD_API_KEY_SHA256` and `VISUAL_AGENT_CLOUD_API_KEY_SALT` before enabling protected endpoints. If either value is missing, protected token, publish, delete, run, run-status, and artifact requests fail closed with HTTP 503. Health remains anonymous. Anonymous marketplace list/search requests, and detail/download requests for workflows marked `public`, remain available; private workflow detail/download requires a valid bearer token.

One Cloud API process is a single security tenant. Bind its credential to server-controlled `VISUAL_AGENT_CLOUD_ORG` and `VISUAL_AGENT_CLOUD_USER` values; request `X-Visual-Agent-Org` / `X-Visual-Agent-User` headers and body identity fields may only repeat those values and cannot select another tenant or user. Conflicting claims return HTTP 403. Configure `VISUAL_AGENT_CLOUD_RUN_PROFILE` as the maximum permitted execution profile. It defaults to `dry-run`, and a request cannot raise that server-side limit. Generate and distribute the plaintext client token separately, then store only its salted hash on the API service.
...
For loopback-only development, `cloud-server` request authentication is optional. Start it with `--api-key-env CHECKPOINT_CLOUD_SERVER_API_KEY` (the default environment variable name) or `--api-key`, and non-health endpoints require `Authorization: Bearer <token>`. Binding to a non-loopback host such as `0.0.0.0` is refused unless an API key is configured. Add `--required-org <org>` to require a matching `X-Visual-Agent-Org` header after bearer-token authentication; `--required-org` is an additional scope check and never replaces the API key for non-loopback exposure. The server startup output only reports whether auth/org checks are enabled; it does not print the token.
