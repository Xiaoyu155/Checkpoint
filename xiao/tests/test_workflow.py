...
def test_run_command_dry_run_does_not_execute(tmp_path) -> None:
    ...
    result = WorkflowRuntime(output_dir=tmp_path / "runs").run(workflow, run_profile="dry-run")

    assert result.steps[0].status == ActionStatus.DRY_RUN
    assert not marker.exists()


def test_run_command_supervised_executes_and_records_receipt(tmp_path) -> None:
    workflow = Workflow(
        name="command",
        version=1,
        steps=(
            WorkflowStep(
                ...
                action="run_command",
                params={
                    "command": "python -c \"print('tool smoke ok')\"",
                    "expect_text": "tool smoke ok",
                    "description": "print smoke",
                },
            ),
        ),
    )

    result = WorkflowRuntime(output_dir=tmp_path / "runs").run(workflow, run_profile="supervised")
    step = result.steps[0]

    assert step.status == ActionStatus.SUCCESS
    assert step.metadata["exit_code"] == 0
    assert validate_operation_receipt(step).valid is True


def test_run_command_fails_on_unexpected_exit_code(tmp_path) -> None:
    workflow = Workflow(
        name="command",
        version=1,
        steps=(
    ...
    assert "exit code 7 != 0" in result.steps[0].message
...
def test_new_assertion_helpers_cover_element_url_count_attribute_and_overlap() -> None:
    ...
    class DummyLocator:
        def __init__(self, count: int, attrs: dict[str, Any], calls: list[tuple[Any, ...]]) -> None:
            self._count = count
            self._attrs = attrs
            self._calls = calls
    ...
    class DummyPage:
        ...
        def locator(self, selector: str) -> DummyLocator:
            ...
            return DummyLocator(counts.get(selector, 0), attrs, self.calls)

        def wait_for_url(self, pattern, *, timeout: int) -> None:
            self.calls.append(("url_wait", pattern.pattern, timeout))
            if pattern.search(self.url) is None:
                raise TimeoutError("URL missing")
    ...
    assert ("locator_wait", "attached", 5000) in page.calls
    assert ("url_wait", "/dashboard", 5000) in page.calls
